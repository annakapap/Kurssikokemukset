...
def list_experiences(q: str | None = None):
    db = get_db()
    if q:
        return db.execute(
            """
            SELECT
                e.id, e.user_id, e.course_name, e.content, e.created_at, e.updated_at,
                u.username
            FROM experiences e
...
def create_experience(user_id: int, course_name: str, content: str) -> None:
    db = get_db()
    db.execute(
...
def get_experience_by_id(experience_id: int):
    ...
    return db.execute(
        """
        SELECT id, user_id, course_name, content, created_at, updated_at
        FROM experiences
        WHERE id = ?
        """,
        (experience_id,),
...
def update_experience(experience_id: int, course_name: str, content: str) -> None:
    db = get_db()
    db.execute(
        """
        UPDATE experiences
